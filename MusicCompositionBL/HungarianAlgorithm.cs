...
namespace MusicCompositionBL
{
    ...
    public static class HungarianAlgorithm
    {
        ...
        /// </summary>
        /// <param name="costsMat">A cost matrix; the element at row <em>i</em> and column <em>j</em> represents the cost of agent <em>i</em> performing task <em>j</em>.</param>
        /// <returns>A matrix of assignments; the value of element <em>i</em> is the column of the task assigned to agent <em>i</em>.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="costsMat"/> is null.</exception>
        public static int[] FindAssignments(int[,] costsMat)
        {
            int[,] costs = costsMat;
            if (costs == null)
            ...
            for (var i = 0; i < h; i++)
            {
                ...
                //find the nim number in the row
                for (var j = 0; j < w; j++)
                {
